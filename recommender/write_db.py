    ...
    try:
        ...
        with conn.cursor() as cursor:
            for recommended_id, similarity_score in recommended_id_similarity_scores:
                cursor.execute(f"SELECT RS_SendFriendRecommendation({user_id}, {recommended_id}, {similarity_score})")
            conn.commit()
        return True
...
def send_group_recommendations_to_db(connection_pool, group_id, recommended_id_similarity_scores):
    ...
    try:
        ...
        with conn.cursor() as cursor:
            for recommended_id, similarity_score in recommended_id_similarity_scores:
                cursor.execute(f"SELECT RS_SendGroupRecommendation({group_id}, {recommended_id}, {similarity_score})")
            conn.commit()
        return True
    except (DatabaseError, OperationalError) as e:
...
def insert_new_group(connection_pool, group_name, hobby_rating_list):
    ...
    try:
        conn = connection_pool.getconn() # Get a connection from the pool
        with conn.cursor() as cursor:
            cursor.execute(f"SELECT RS_CreateGroup('{group_name}', {hobby_rating_list[0]}, {hobby_rating_list[1]}, {hobby_rating_list[2]}, {hobby_rating_list[3]}, {hobby_rating_list[4]})")
            conn.commit()
        return True
    except (DatabaseError, OperationalError) as e:
